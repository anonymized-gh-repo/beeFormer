...
parser.add_argument("--tuning_epochs", default=1, type=int, help="Final lr epochs, only if scheduler is LinearWarmup")
parser.add_argument("--epochs", default=1, type=int, help="Final lr epochs, only if scheduler is none")
...
parser.add_argument("--top_k", default=0, type=int, help="Optimize only for top-k predictions on the output of the model")
...
# evaluate
parser.add_argument("--evaluate", default="false", type=str, help="final evaluation after training [true/false]")
parser.add_argument("--evaluate_epoch", default="false", type=str, help="evaluation after every epoch [true/false]")
parser.add_argument("--save_every_epoch", default="true", type=str, help="save after every epoch [true/false]")
...
def main(args):
    ...
    if args.dataset in config.keys():
        ...
        full_train_interactions = pd.concat([df,df2])
    ...
    if args.dataset == "goodlens":
        cc = np.array(full_train_interactions.item_id.cat.categories)
    else:
    ...
    if args.dataset == "goodlens":
        X = get_sparse_matrix_from_dataframe(full_train_interactions)
    else:
