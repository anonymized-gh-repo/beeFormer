...
class simpleBee(keras.models.Model):
    """
    Same as KerasELSA but receives data from SparseRecSysDataset - data is batch of user vectors + slicer for nonzero entries
    """
    def __init__(self, B, n_dims, n_hidden, items_idx, device, top_k=0):
        super().__init__()
        #self.A = torch.nn.Parameter(torch.nn.init.xavier_uniform_(torch.empty([n_items, n_dims])))
        self.device = device
        self.B = torch.from_numpy(B).to(device)
        self.hidden = torch.nn.Linear(B.shape[1], n_hidden, bias=False, device=device)
        self.relu = torch.nn.ReLU()
        self.transform = torch.nn.Linear(n_hidden, n_dims, bias=False, device=device)
        self.tanh = torch.nn.Tanhshrink() #torch.nn.Tanh()
        self.items_idx = items_idx
        self(np.zeros([1,B.shape[0]]))
        self.finetuning = False
        self.top_k = top_k

    def A(self):
        A = self.hidden(self.B)
        A = self.relu(A)
        A = self.transform(A)
        A = self.tanh(A)
    ...
    def predict_df(self, df, k=100, user_ids=None, candidates_df=None):
...
class NMSEbeeformer(keras.models.Model):
    ...
    def train_step(self, data):
        ...
        # we are doing this once during init of dataloader
        # tokenized_items = {k:v.to(DEVICE) for k,v in tokenized_items.items()}
        ...
        # ??? scale the loss for expected number of accumalation steps
        # tohle nevim jestli mam udelat nebo ne
        # loss = loss/max_i

        # Call torch.Tensor.backward() on the loss to compute gradients
        # for the gradient checkpoint (our ELSA A matrix).
        loss.backward()
        # logger.info("Second forward pass")
        # sbert forward pass #2
        # now we will do the sbert forward pass again, but this time we will track gradients
        # this time, for memory reasons in batches
        batched_results=[]
        for i in range(max_i):
            ind = i*sbert_batch_size
            ind_min = ind
