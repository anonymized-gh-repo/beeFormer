...
class beeformerDataset(keras.utils.PyDataset):
    ...
    """
    def __init__(self, X: scipy.sparse.csr_matrix, tokenized_sentences, device, batch_size: int = 1024, shuffle=False, workers=1, use_multiprocessing=False, max_queue_size=10, max_output=None):
        super().__init__(workers=workers, use_multiprocessing=use_multiprocessing,
                         max_queue_size=max_queue_size)
        self.X, self.batch_size, self.shuffle, self.tokenized_sentences = X, batch_size, shuffle, {k:v.to(device) for k,v in tokenized_sentences.items()}
    ...
    def __getitem__(self, n):
        ...
        return (torch_coo_x.to(self.device).to_dense(), torch_coo_y.to(self.device).to_dense()), (tokenized_items, torch.from_numpy(slicer).long(), torch.from_numpy(slicer_neg).long())
